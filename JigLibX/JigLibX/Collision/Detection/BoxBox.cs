            ...
            float oldDepth = depth + bodyDeltaLen;
              ...
              SmallCollPointInfo[] collPtArray = SCPIStackAlloc();
                ...
                if (numPts > 0) {
                  ...
                  for (i = 0; i < numPts; ++i) {
                    ...
                    if (contactPointsFromOld) {
                      if (numCollPts < MaxLocalStackSCPI) {
                        ...
                        collPtArray[numCollPts].R0 = contactPts[i].Pos - body0OldPos;
                        collPtArray[numCollPts].R1 = contactPts[i].Pos - body1OldPos;
                        collPtArray[numCollPts++].InitialPenetration = depth;
                      }
                    } else {
                      if (numCollPts < MaxLocalStackSCPI) {
                        // BEN-OPTIMISATION: Instead of allocating a new SmallCollPointInfo we reuse the existing one.
                        collPtArray[numCollPts].R0 = contactPts[i].Pos - body0NewPos;
                        collPtArray[numCollPts].R1 = contactPts[i].Pos - body1NewPos;
                        collPtArray[numCollPts++].InitialPenetration = depth;
                      }
                    }
                  ...
                  if (numCollPts < MaxLocalStackSCPI) {
                    // BEN-OPTIMISATION: Instead of allocating a new SmallCollPointInfo we reuse the existing one.
                    collPtArray[numCollPts].R0 = cp0;
                    collPtArray[numCollPts].R1 = cp1;
                ...
                // report Collisions
                collisionFunctor.CollisionNotify(ref info, ref N, collPtArray, numCollPts);
