                ...
                SmallCollPointInfo[] collPtArray = SCPIStackAlloc();
                    if (numPts > 0)
                    {
                        ...
                        for (i = 0; i < numPts; ++i)
                        {
                            ...
                            collPtArray[i].R0.X = pts[i].X - boxNewPos.X;
                            collPtArray[i].R0.Y = pts[i].Y - boxNewPos.Y;
                            collPtArray[i].R0.Z = pts[i].Z - boxNewPos.Z;

                            collPtArray[i].R1.X = pts[i].X - meshPos.X;
                            collPtArray[i].R1.Y = pts[i].Y - meshPos.Y;
                            collPtArray[i].R1.Z = pts[i].Z - meshPos.Z;

                            collPtArray[i].InitialPenetration = oldDepth;
                        }

                        collisionFunctor.CollisionNotify(ref info, ref N, collPtArray, numPts);
