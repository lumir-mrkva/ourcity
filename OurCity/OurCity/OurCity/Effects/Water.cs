...
namespace OurCity.Effects
{
    public class Water : DrawableGameComponent
    {
        ...
        public Water(Game game, string Environment, Vector3 position, Vector3 scale)
            : base(game)
        {
            myWidth = 128;
            myHeight = 128;
        ...
        {
            effect = Game.Content.Load<Effect>("Map/Shaders/Water");

            effect.Parameters["tEnvMap"].SetValue(Game.Content.Load<TextureCube>(EnvAsset));
            effect.Parameters["tNormalMap"].SetValue(Game.Content.Load<Texture2D>("Map/Textury/Water/waves2"));

            //myPosition = new Vector3(basePosition.X - (myWidth / 2), basePosition.Y, basePosition.Z - (myHeight / 2));
            //myPosition = new Vector3(-128 * 10, -70, -128);
