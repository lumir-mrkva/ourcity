...
namespace OurCityEngine.Cameras
{
    ...
    public class Camera : GameComponent
    {
        ...
        float amountOfMovementInside = 50.0f;
        ...
        private float nearPlaneDistance = 1.1f;
        ...
        {
            // disable if not default
            if (Enabled && (CameraManager.Instance.Cameras["Default"] == this))
            {
                ...
                if (keys.IsKeyDown(Keys.Q))
                    amountOfMovementInside -= 0.1f;
                if (keys.IsKeyDown(Keys.E))
                    amountOfMovementInside += 0.1f;
                
                ProcessInput((float)elapsedTime * amountOfMovementInside);
                UpdateView();
